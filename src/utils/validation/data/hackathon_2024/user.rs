...
pub fn field(new_user: &HackathonUser2024Insertable) -> Result<(), ApiError> {
    if !new_user.email.is_email() {
        return Err(ApiError::ValidationError(
            format!("Email don't correct {}", new_user.email).to_owned(),
        ));
    }

    if !new_user.phone.is_phone() {
        return Err(ApiError::ValidationError(
            format!("Phone don't correct {}", new_user.phone).to_owned(),
        ));
    }
    ...
    if !new_user.first_name.less_for(name_length) || !new_user.last_name.less_for(name_length) {
        return Err(ApiError::ValidationError(
            format!("First name or last name greater for {} symbol", name_length).to_owned(),
        ));
    }
    Ok(())
}
