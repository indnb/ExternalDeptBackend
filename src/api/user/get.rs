...
pub async fn get_role(
    ...
    match user_role {
        ...
        Err(diesel::result::Error::NotFound) => Err(ApiError::Unauthorized("Claims mismatched".to_string())),
        Err(_) => Err(ApiError::InternalServerError),
    }
}
#[get("/user/get_authorization_user")]
pub async fn get_authorization_user(db_pool: &State<DbPool>, claims: Claims) -> Result<Json<UserQueryable>, ApiError> {
    let mut connection = get_connection(db_pool)?;
    ...
    match user {
        ...
        Err(diesel::result::Error::NotFound) => Err(ApiError::Unauthorized("Token or mismatched".to_string())),
