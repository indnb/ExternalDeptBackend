use crate::{
    dto::response::hackathon_2025::university::update_university_cached, utils::prelude_api::*,
};
use rocket::delete;
...
#[delete("/hackathon_2025/university/by_id/<id>")]
pub async fn by_id(db_pool: &DbState, id: i32, admin_match: AdminAuthData) -> Result<(), ApiError> {
    admin_match.check_admin()?;
    let id = crate::diesel::utils::hackathon_2025::university::delete::by_id(db_pool, id)?;

    update_university_cached(db_pool).await?;

    info!("Successfully deleted university from hackathon_university_2025 with id {id}");
