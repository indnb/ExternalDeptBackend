...
#[post("/hackathon_2024/university/create_university", data = "<university_data>")]
pub async fn create_university(db_pool: &State<DbPool>, university_data: Json<University<'_>>, admin_match: AdminMatch) -> Result<String, ApiError> {
    let mut  db_connection = get_connection(db_pool)?;
    let university = university_data.into_inner();
    admin_match.check_admin()?;

    let university_id = diesel::insert_into(crate::diesel::schema::hackathon_university_2024::table)
        .values(university)
